...
def _get_default_serial_port_candidates() -> Iterable[str]:
    ...
    if is_cygwin() or is_wsl():
        candidates.append("/dev/ttyS*")

    return candidates


def _should_use_globs() -> bool:
    """Returns whether we should use globs when resolving serial ports in the
    current platform.
    """
    return is_cygwin() or is_linux() or is_macos()
...
"""Helper table for the `crc32` function."""
# fmt: on


def crc32(data: Iterable[int], state: int = 0) -> int:
    """Calculates the CRC32 checksum of the given chunk of data, starting from
    the initial seed state.
    """
    for byte in data:
        index = (state ^ byte) & 0xFF
        state = _crctab[index] ^ (state >> 8)
    return state
...
def get_serial_port_candidates(seed: Iterable[str] | None = None) -> Iterable[str]:
    ...
    the uploader is running on.

    This function is adapted from the ``ports_to_try()`` function of the original
    ArduPilot uploader script.

    Parameters:
        seed: iterable of glob patterns representing possible serial port
            candidates to try
    ...
    # prevents the upload from causing modem hangups etc.
    if _should_use_globs():
        port_list = sum((glob(pattern) for pattern in patterns), [])
    else:
        port_list = []

    # Exclude ports that do not seem to make sense on the current platform
    system_str = system().lower()
    if "cygwin" in system_str:
        # Anything can happen on Cygwin so we don't filter
        pass
    elif "darwin" in system_str:
        # Exclude ports that look like Windows or Linux ports
        port_list = [
            port for port in port_list if "COM" not in port and "ACM" not in port
        ]
    elif "win" in system_str:
        # Exclude ports that look like from POSIX systems
        port_list = [port for port in port_list if "/" not in port]

    return port_list


def iter_chunks(
    seq: Sequence[T], length: int, *, start: int = 0
) -> Iterable[Sequence[T]]:
    """Iterates over fixed-size chunks of the input sequence.

    Parameters:
        seq: the input sequence
        length: the size of each chunk (except maybe the last one)
